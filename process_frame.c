...
for(r=nc; r<nr*nc;r+=nc){
	...
	//data.u8TempImage[BACKGROUND][r+c]=(uint8)MIN(255,MAX(0,((avgDxy[1][r+c]) >> 10)));
	//data.u8TempImage[BACKGROUND][r+c]=(uint8)MIN(255,MAX(0,128+((avgDxy[2][r+c]) >> 4)));
		}
	}
}
...
for(r = nc*(Border+1); r < nr*nc-nc * (Border+1); r+= nc) {// we skip first and last lines (empty)
	...
	//do pointer arithmetics with respect to center pixel location
		...
		//now averaged
		avgDxy[Index][r+c] = (sy >> 8); // max value 2²²

		//data.u8TempImage[THRESHOLD][r+c]=(uint8)MIN(255,MAX(0,((avgDxy[Index][r+c]) >> 11)));
	}
}
	...
	int c,r;
			for(r=nc*(Border+1); r<nr*nc-nc*(Border+1);r+=nc){
				for(c=Border+1; c<nc-(Border+1);c++){
					...
					avgDxy[3][r+c] = (dIx2*dIy2-dIxy*dIxy)-((k*(dIx2+dIy2)*(dIx2+dIy2))>>7);

					data.u8TempImage[THRESHOLD][r+c]=(uint8)MIN(255,MAX(0,avgDxy[3][r+c]>>7));
				}
			}
}
...
void MaxFinder()
{
	...
	//Set max in x-Direction
	for(r = nc*(Border+1); r < nr*nc-nc*(Border+1); r+= nc) {// we skip first and last lines (empty)
		for(c = Border+1; c < nc-(Border+1); c++) {// +1 because we have one empty border column
		// do pointer arithmetics with respect to center pixel location
				int* p = &avgDxy[3][r+c];
				// Find max Value from p-6 to p+6
	...
	for(r = nc*(Border+1); r < nr*nc-nc * (Border+1); r+= nc) {// we skip first and last lines (empty)
		...
		//do pointer arithmetics with respect to center pixel location
			...
			if(avgDxy[4][r+c] == avgDxy[3][r+c] && avgDxy[3][r+c]>threshold){
				DrawBoundingBox(c-SizeBox,r/nc+SizeBox,c+SizeBox,r/nc-SizeBox,0,GREEN);

			}
			data.u8TempImage[BACKGROUND][r+c]=(uint8)MIN(255,MAX(0,((avgDxy[4][r+c])>>7)));
		}
	}
}
